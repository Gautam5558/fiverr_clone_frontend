...
const Gig = () => {
    ...
    const { isLoading, data, error, refetch } = useQuery({
        queryKey: [`${gigId}`],
        queryFn: () => axios.get("http://localhost:3000/api/gigs/single/" + gigId).then((res) => { return res.data })
    })
    ...
    const { isLoading: loadingUser, error: userError, data: userData } = useQuery({
        ...
        queryFn: () => axios.get("http://localhost:3000/api/users/" + userId).then((res) => { return res.data }),
        enabled: !!userId
    })
    ...
    const handleClick = (e) => {
        if (user._id) {
            ...
            if (user.isSeller) {
                setPaymentError("You cannot buy the gig as you are registered as a seller")
            }
            else {
                navigate("/pay/" + gigId)
            }
    ...
    const handleContact = async (sellerId, buyer) => {
        if (buyer._id) {
            if (buyer.isSeller) {
                setContactError("You cannot contact the seller as you yourself are a seller")
            }
            else {
            ...
            <div className="container">
                <div className="left">
                    ...
                    <div className="user">
                        ...
                        <div className="stars">
                            {data.totalStars != 0 && Array(Math.round(data.totalStars / data.starNumber)).fill().map((item, index) => {
                                return (
                                    <img src='/images/star.png' key={index} />
                                )
                            })}
                            <span>{data.totalStars != 0 && Math.round(data.totalStars / data.starNumber)}</span>
                        </div>
                    </div>
                    ...
                    <div className="seller">
                        ...
                        <div className="information">
                            ...
                            <div className="data">
                                ...
                                <button onClick={(e) => handleContact(data.userId, user)} >Contact Me</button>
                            </div>
                        </div>
                        {contactError && <span className='ContactError'>{contactError}</span>}
                        <div className="box">
                            <div className="items">
                                <div className="item">
